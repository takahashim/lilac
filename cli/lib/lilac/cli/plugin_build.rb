...
module Lilac
  module CLI
    # Compile one or more pure-Ruby plug-in source files (mrblib-style:
    # `Lilac::Directives::Scanner.register_directive(...) { ... }`) into
    # a single `.mrb` bytecode file that can be loaded into a running
    ...
    # difference from `build` is that plug-in `.mrb` files have a
    # user-specified output path (no content-hash filename) and are
    # never wrapped with `Lilac.start` / framework boot — they're
    # library-style code that registers directives at load time and
    # then yields control back.
    #
    ...
    class PluginBuild
      ...
      def initialize(inputs:, output:, mrbc_path: nil)
        raise Error, 'plugin-build: at least one input file required' if inputs.empty?
      ...
      def aggregate_sources
        @inputs.map do |path|
          raise Error, "plugin-build: input file not found: #{path}" unless File.file?(path)
