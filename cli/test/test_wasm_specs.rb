...
#
# Skips cleanly when `build/lilac-full.wasm` is absent (CI without
# `make lilac-full`) or when `MRUBY_WASM_RUNTIME_PATH` is unset.
  ...
  SpecRunner::PURE_SPECS.each do |spec_rel|
    define_method("test_#{spec_rel.tr('/', '_').sub('.rb', '')}") do
      ...
      unless File.file?(MrubyWasm::DEFAULT_WASM_PATH)
        skip "build/lilac-full.wasm not built (run `make lilac-full`)"
      end
