...
class TestPluginDiscovery < Minitest::Test
  def setup
    @tmp = Dir.mktmpdir("lilac-plugin-discovery-test")
  end
  ...
  # API surface for `PluginDiscovery` to inspect. Easier than spinning
  ...
  def stub_bundler_specs(specs)
    ...
    # block so PluginDiscovery sees our stub instead of any real one
  ...
  def make_plugin_gem(name, mrblib_files)
    ...
    FakeSpec.new(
      name: name,
      version: "0.1.0",
      metadata: { "lilac_plugin" => "true" },
      full_gem_path: base,
    )
  end

  def test_returns_empty_when_no_plugins_in_bundle
    stub_bundler_specs([]) do
      assert_empty Lilac::CLI::PluginDiscovery.run
    end
  end

  def test_returns_empty_when_no_specs_have_lilac_plugin_metadata
    other = FakeSpec.new(
      name: "some-other-gem",
      version: "1.0.0",
  ...
  def test_picks_up_plugin_gems_and_lists_mrblib_files
    plugin = make_plugin_gem("lilac-plugin-foo", {
                               "a.rb" => "# a",
                               "b.rb" => "# b",
                             })
    stub_bundler_specs([plugin]) do
      results = Lilac::CLI::PluginDiscovery.run
      assert_equal 1, results.length
      assert_equal "lilac-plugin-foo", results.first.name
  ...
  def test_skips_plugin_gem_with_empty_mrblib
    empty_plugin = FakeSpec.new(
      name: "lilac-plugin-empty",
      version: "0.1.0",
      metadata: { "lilac_plugin" => "true" },
      full_gem_path: File.join(@tmp, "empty"),
    )
    # `full_gem_path` exists but no mrblib/ dir → no source to compile.
    # Skipping rather than erroring keeps a borked plug-in from breaking
    # the whole build; the user still sees no .mrb produced from this gem.
    FileUtils.mkdir_p(empty_plugin.full_gem_path)
    stub_bundler_specs([empty_plugin]) do
      assert_empty Lilac::CLI::PluginDiscovery.run
    end
  end
