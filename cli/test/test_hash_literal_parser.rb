...
class TestHashLiteralParser < Minitest::Test
  ...
  def test_empty_hash
    assert_equal [], Lilac::CLI::HashLiteralParser.parse("{}")
    assert_equal [], Lilac::CLI::HashLiteralParser.parse("{ }")
    assert_equal [], Lilac::CLI::HashLiteralParser.parse("  {  }  ")
  end

  def test_single_bare_pair
    assert_equal [["active", "@is_active"]], Lilac::CLI::HashLiteralParser.parse("{ active: @is_active }")
  end

  def test_multiple_bare_pairs
    pairs = Lilac::CLI::HashLiteralParser.parse("{ active: @a, error: @e, loading: @l }")
    assert_equal [["active", "@a"], ["error", "@e"], ["loading", "@l"]], pairs
  end

  def test_single_quoted_kebab_key
    assert_equal [["btn-primary", "@primary"]], Lilac::CLI::HashLiteralParser.parse("{ 'btn-primary': @primary }")
  end
  ...
  def test_quoted_tailwind_variant_with_colon_in_key
    pairs = Lilac::CLI::HashLiteralParser.parse("{ 'hover:bg-blue-500': @h, 'md:text-lg': @d }")
    assert_equal [["hover:bg-blue-500", "@h"], ["md:text-lg", "@d"]], pairs
  end

  def test_quoted_bem_key
    assert_equal [["card__title--large", "@show"]], Lilac::CLI::HashLiteralParser.parse("{ 'card__title--large': @show }")
  end

  def test_quoted_tailwind_arbitrary_value_key
    assert_equal [["top-[117px]", "@t"]], Lilac::CLI::HashLiteralParser.parse("{ 'top-[117px]': @t }")
    assert_equal [["bg-[#bada55]", "@b"]], Lilac::CLI::HashLiteralParser.parse("{ 'bg-[#bada55]': @b }")
  end

  def test_mixed_bare_and_quoted
    pairs = Lilac::CLI::HashLiteralParser.parse("{ active: @a, disabled: @d, 'btn-primary': @p }")
  ...
  def test_extra_whitespace_around_colons_and_commas
    pairs = Lilac::CLI::HashLiteralParser.parse("{   active   :   @a   ,   error  :  @e   }")
    assert_equal [["active", "@a"], ["error", "@e"]], pairs
  end

  def test_newlines_between_entries
    pairs = Lilac::CLI::HashLiteralParser.parse(<<~HASH)
      {
        active: @a,
        error: @e
      }
  ...
  def test_bare_key_with_kebab_is_rejected
    err = assert_raises(Lilac::CLI::HashLiteralParser::Error) { Lilac::CLI::HashLiteralParser.parse("{ btn-primary: @p }") }
  ...
  def test_semicolon_in_quoted_key_is_rejected
    err = assert_raises(Lilac::CLI::HashLiteralParser::Error) { Lilac::CLI::HashLiteralParser.parse("{ 'a;b': @p }") }
    assert_includes err.message, "invalid character"
  end

  def test_control_char_in_quoted_key_is_rejected
    err = assert_raises(Lilac::CLI::HashLiteralParser::Error) { Lilac::CLI::HashLiteralParser.parse(%({ "a\x01b": @p })) }
    assert_includes err.message, "invalid character"
  end
  ...
  def test_missing_value_is_rejected
    assert_raises(Lilac::CLI::HashLiteralParser::Error) { Lilac::CLI::HashLiteralParser.parse("{ active: }") }
  end

  def test_missing_colon_is_rejected
    assert_raises(Lilac::CLI::HashLiteralParser::Error) { Lilac::CLI::HashLiteralParser.parse("{ active @a }") }
  end

  def test_trailing_content_after_close_brace_is_rejected
    err = assert_raises(Lilac::CLI::HashLiteralParser::Error) { Lilac::CLI::HashLiteralParser.parse("{ active: @a } junk") }
