...
- ~~mruby-wasm-runtime ↔ lilac-wasm-bin の release 同期 flow の整備。~~ → §32.1 で解消。

## 32.1 Refinement: gem は RELEASE wasm を同梱(2026-06-24)

初版の `rake build:assets` は `build/lilac-{full,compiled}.wasm` /
`build/mrbc-host.wasm`(= **dev ビルド**、`-Oz`/`--strip-debug` 無し)を
`data/` にコピーしていた。dev の `lilac-full.wasm` は 4.3MB あり、release
(`*.release.wasm`、832KB)に対して約5倍。配布 gem が debug 情報入りの巨大
binary を運ぶのは不適で、consumer(`lilac build` の vendoring、および gem を
直接使う埋め込み消費者)の dist を不必要に肥大させていた。

dev/release の差は **`-Oz` + `-Wl,--strip-debug` のみ**(機能・挙動・ABI は
同一。`build_config/lilac-full.rb` 参照)。release を配っても挙動リスクは無い。

- `wasm-bin/Rakefile build:assets`: コピー元を `*.release.wasm` に変更
  (`data/` の正規名 `lilac-full.wasm` 等は据え置き)。要 `make lilac-*-release`。
- `wasm-bin/lib/lilac/wasm/bin.rb`: monorepo フォールバックを
  `data/<name>.wasm` → `build/<name>.release.wasm` → `build/<name>.wasm`(dev)
  の3段に。`path:` gem(data/ 空)でも release を拾い、無ければ dev に落ちる。
- 検証: release `mrbc-host.wasm` が wasmtime-rb 経由で `compile_source` を駆動し
  `.mrb` を生成できること(`lilac build --target compiled`、wasm backend)を確認済み。

サイズ実測: full 4.3MB→832KB、compiled 3.06MB→584KB、mrbc-host→459KB。
