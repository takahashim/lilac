...
| [0033](./0033-js-tests-on-wasmtime-rb-dommy-quickjs.md) | JS 依存テストを wasmtime-rb + Dommy(dommy-js-quickjs)へ移行。async スタブを実 QuickJS イベントループに置換、bundle/parity を Ruby spec 化 | `test/ruby_spec/mruby_wasm.rb` + `quickjs_bridge.rb` + `cli/test/test_bundle_runtime.rb` + `test_parity.rb` + `Makefile` | 完了 (2026-05-31、Phase 1+2 / gem 依存の published 切替は後続) |
