...
「runtime canonical / CLI は optional な optimization layer」(decisions §1)
の原則に照らすと、**plugin 作成時に CLI のことは考えなくて済む** のが理想。
今の状態は plugin author に CLI 側の知識まで要求している。
...
module Lilac
  module Extras
    ...
    def self.hooks_tooltip(scanner, raw_value, el, item)
    ...
    def self.hooks_autofocus(_scanner, _raw, el, _item)
      el.call(:focus)
    end
  end
end
...
- `register_named_directive("foo-bar")` は `data-foo-bar` 属性を扱う
  directive として登録される
- dispatch 時、**呼び出し元 module の `hooks_foo_bar` メソッド** が
...
Lilac::Extras.hooks_tooltip(__scanner_proxy_for(refs.lil0), "@msg", refs.lil0.to_js, nil)
```
...
4. **デバッグ容易**: スタックトレースが `Lilac::Extras.hooks_tooltip` を
   直接指す。block 内の匿名 proc より遥かに追いやすい
5. **テスト容易**: メソッドを直接呼べる。Scanner mock を作るだけで
   unit test 可能
6. **読みやすい規約**: 「`data-X` の実装は `hooks_X` メソッド」というルールは
...
#### (a) handler module の決め方

3 通り:

1. **`register_named_directive` の呼び出し元 module (`self`) を使う**
   — 一番簡潔。CLI が AST から `module ... ; register_named_directive ; end`
   の入れ子で constant path を読めば良い
2. **明示指定**: `register_named_directive("tooltip", handler: Extras)`
3. **メソッド参照を渡す**:
   `register_named_directive("tooltip", &Extras.method(:hooks_tooltip))`

第一版は **(1) をデフォルト、(2) を escape hatch** として両方サポート。
(3) は冗長すぎるので採用しない。

#### (b) captures_name (例: `data-on-X`) の扱い
...
```ruby
def self.hooks_X(scanner, raw_value, el, item)
```

の 4 引数固定。`name`(captures_name 用)と `descriptor`(エラーメッセージ用)
は当面省略。これらが要る directive は block 版で書く。

#### (d) `register_directive` (block 版) との共存

完全に置き換えず、両方残す:

- `register_directive` (block 版): 低レベル、自由、CLI 最適化は乗らない
  (= runtime delegate 経路へ流れる必要がある)
- `register_named_directive` (convention 版): 高レベル、規約に縛られる、
  CLI が自動配線

built-in (`mruby-lilac-form`) は **当面 block 版のまま**。migrate するかは
別議論(API breaking change の余地があるため pre-1.0 期の判断保留)。

#### (e) runtime fallthrough の必要性

block 版 directive を持つ plugin 向けに **runtime fallthrough**
(generated `bind_template_hook` の末尾で Scanner.scan_extension_directives
を呼ぶ仕組み、これも未実装) も併設すべきか。

- 入れる: block 版 plugin も lilac-compiled で動く
- 入れない: block 版 plugin は lilac-full でしか動かない
  (compiled は CLI codegen が必要)

第一版は **入れない** で良い。`register_named_directive` で書ける範囲なら
両 variant で動く、それ以外は lilac-full のみという制約は許容できる。
需要が顕在化したら別 proposal で追加する。

### 実装的課題

#### Runtime 側

- `Scanner.register_named_directive(name, handler: nil)` を 1 メソッド追加
  (~20 行):
  - kebab-case `name` → snake_case メソッド名 (`hooks_<name>`)
  - handler 省略時は **`Kernel#caller_locations` で呼び出し元ファイルを
    特定 → 該当ファイルが定義する module を `self` 経由で得る**
    のは mruby では難しいかもしれない。実装的には `register_named_directive`
    が呼ばれた時点の `caller binding` の `Module.nesting.first` を使う等
    のテクニックが必要。要 PoC
  - 代替案: 呼び出し元 module を `register_named_directive` の引数として
    明示渡し or `module Foo; register_named_directive ...; end` の中で
    `self` を捕捉する API 形に
- `register_directive` (block 版) は変更なし、共存

#### CLI 側

- 起動時 (`builder.rb` の require 直後) に
  `runtime/mruby-lilac-*/mrblib/**/*.rb` を Prism スキャン
- `Lilac::Directives::Scanner.register_named_directive("X")` の call node
  を抽出し、`{ "X" => "Lilac::Foo" }` のテーブル化
- このスキャン結果から `TemplateAST.register_directive` と
  `Codegen.register_emitter` を自動 call
- スキャンは build 起動時 1 回。`runtime/` の更新があれば再起動が必要だが
  通常開発フローで問題なし
- gemspec 上、`spec.files` に `runtime/mruby-lilac-*/mrblib/**/*.rb` を
  含めるかは別議論(現状 cli/ gemspec は `lib/` 配下しか含まないため、
  gem install 経由の利用パスは未対応)
...
- **§1 (Runtime canonical 化)**: 「runtime が canonical、CLI は optional
  optimization layer」を強化する。plugin author は CLI のことを考えなくて
  良くなる
- **§17 (directive binding は codegen が canonical / scanner gem は grammar
  reference + duplicate pair の SSOT)**: 本提案は scanner gem 側 plugin
  と CLI side の二重実装を、命名規約 + AST 読み取りで自動同期する仕組み。
  §17 の SSOT 原則を plug-in 領域に拡張するもの
...
1. mruby で `Module.nesting` 相当が使えるか (handler module の自動解決)
2. Prism による mrblib スキャンの起動時コスト (推測: 数十ファイルなら ~100ms)
3. captures_name 系の本当の需要 (= 第一版で省略して問題ないか)
4. 既存 form の migrate を強制する流れになった場合の API breaking range

PoC (実装プロトタイプ) を作って (1)-(2) を実証してから正式判断する流れが
妥当。
