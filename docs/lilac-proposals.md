...
## Plug-in .mrb の hot-reload (`lilac dev` 統合 or `lilac plugin-build --watch`)

### 動機

decisions §23 / §24 で plug-in 配布形態が確立した結果、**plug-in 作者の
dev loop** が明確に見えてきた:

- plug-in source (`.rb`) を編集
- `lilac plugin-build` を手動で再実行 (面倒)
- example app の dev server をブラウザで手動 reload (さらに面倒)

iteration あたり ~10 秒のマニュアル作業 = plug-in 開発体験が悪い。
公式 plug-in (extras) でも第三者 plug-in でも同じ摩擦。

利用者 (plug-in を黒箱として使うアプリ作者) には影響なし — `.mrb` は
固定なので既存の `lilac dev` で足りる。

### 提案

3 案を検討した結果、**案 B (`plugin-build --watch`) を Phase 1**、
案 A (config 宣言) は需要が出てから Phase 2 として後追い。

#### 案 A — `lilac.config.rb` で plug-in を宣言

```ruby
# example/lilac.config.rb
Lilac::CLI.configure do |c|
  c.plugins = [
    { source: ["../src/plugin.rb"], output: "public/plugin.mrb" },
  ]
end
```

`lilac dev` 起動時に `watched_paths` を拡張し、plug-in source 変更時に
`PluginBuild` を実行 → output 更新 → `live_reload.notify_all`。

長所: declarative、build pipeline が `lilac dev` に集約。
短所: Config の表面積拡大、アプリと plug-in の関心混合。

#### 案 B — `lilac plugin-build --watch` 独立 daemon (Phase 1 推奨)
...
# Terminal 2
lilac plugin-build --watch ../src/plugin.rb -o example/public/plugin.mrb
```
...
#### 案 C — `lilac dev --plugin SRC=OUT` flag

```sh
cd example && lilac dev --plugin ../src/plugin.rb=public/plugin.mrb
```

長所: flag 完結、context 明示。
短所: 複数 plug-in で flag が増える。案 B と機能的同等で UX 劣る。

### 利点

- plug-in 作者の dev loop が「save → reload」に短縮 (= 既存の Lilac dev
  UX と同じレベル)
...
- 手動 `lilac plugin-build` の繰り返し
- shell の `fswatch` / `entr` で代用:
  ```sh
  fswatch -o src/plugin.rb | xargs -n1 -I{} \
    lilac plugin-build src/plugin.rb -o example/public/plugin.mrb
  ```
...
- `PluginBuild#watch(debounce:)` メソッド追加
  - 既存の `Watcher` (`cli/lib/lilac/cli/watcher.rb`) を再利用
  - 各 rebuild 失敗は stderr に出すだけで daemon は継続
- CLI 側に `--watch` flag 追加 (`command.rb` の `plugin_build_opts_parser`)
- 既存 `Watcher` を public API として確定(`require_relative "watcher"`
  を plugin_build.rb から)
- テスト: tempdir で input.rb を変更 → output.mrb の mtime が更新される

Phase 2 (案 A) — 需要次第:

- `Config#plugins` を追加 (Settings に persistent な list)
- `DevServer#watched_paths` を拡張して plug-in sources を含める
- 変更検知 dispatch を「app 系か plug-in 系か」で分岐

### 関連する確定判断

- decisions §23 (plug-in 機構 = runtime fallthrough)
- decisions §24 (plug-in 配布形態 = mrb_load_irep / `boot({ plugins })`)
- §24.6 「後続作業」リストの "lilac dev で plug-in .mrb 変更を即反映" が
  本提案の出発点

### ステータス

未判断 (proposal 段階)。本実装は router / async plug-in 化が落ち着いた
後に検討。**Phase 1 (案 B) は実装規模が小さく、独立した価値があるため、
plug-in が複数増えた段階で着手するのが妥当**。
