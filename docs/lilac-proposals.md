...
## `.lil` bundle fetch — `<link rel="lilac-bundle">` による delivery 戦略

### 動機

ADR-0029 で `<lilac-component>` を廃止し、ランタイムが `<template>` +
`data-component=` を解釈するようになった結果、**ビルドの役割は「`.lil` を
HTML に inject する」だけ**に縮小した。しかし現状の inject 方式には次の
課題が残る:

**(1) ページ HTML が肥大化する**

全 `.lil` の `<template>` + `<script>` がページ HTML の `</body>` 直前に
inject されるため、複数ページ × N コンポーネントだとページ HTML が大きく
なる。ブラウザの first-paint より前にすべてを受信・パースする必要があり、
コンポーネント定義が増えるほど FCP に影響する。

**(2) キャッシュ粒度が粗い**

1 つのコンポーネント定義を更新するとページ HTML 全体が invalidate される。
コンポーネント単位での部分キャッシュが効かない。

**(3) ビルドの責務がまだ HTML 加工に依存**

ADR-0029 で `<lilac-component>` 関連処理は削除したが、`.lil` の中身を
ページ HTML に注入する処理は残っている。これも runtime canonical の精神
からすると「ランタイムが取りに行く」方が筋良い。

**(4) ノービルドユーザーが `.lil` の DRY 機構を使えない**

ADR-0029 で「`<template>` + `data-use=` を直書きすればノービルドでも DRY
可能」になったが、複数ページにまたがる DRY (= `.lil` ファイル経由) はビルド
必須。ノービルドで「コンポーネント定義群を別ファイルに切り出して reuse」
する方法は提供されていない。

### 提案

**`.lil` 群を 1 つの HTML ファイル (`lilac.bundle.html`) に連結し、ページ
HTML からは `<link rel="lilac-bundle">` で参照する** delivery 戦略を新設する。

#### ビルド出力 (`:full` 例)

```
dist/
├── index.html               ← クリーン (参照のみ)
├── lilac.bundle.html        ← 全 .lil を順に連結した 1 ファイル
└── vendor/lilac-full/       ← wasm + bridge (既存)
```

#### `lilac.bundle.html` の中身
...
<template data-template="counter-row">
  <li><span data-text="it.value"></span></li>
</template>
...
```html
<head>
  <link rel="lilac-bundle" href="/lilac.bundle.html">
</head>
<body>
...
`Lilac.start` の collect phase より前に:

1. `document.head` から `<link rel="lilac-bundle">` をすべて検出
2. 各 href を fetch (HTTP/2 なら並列、HTTP/1 でも 1 件なら問題なし)
3. `DOMParser` で `text/html` としてパース
4. parser 結果の `<template>` 要素を `document.body` 末尾に `appendChild`
   - 自動的に `collect_definitions` が拾える
5. `<script type="text/ruby">` を抽出 → **新規 `<script>` 要素を作成 +
   `textContent` をコピー + `document.body` に append**
   - 動的作成 `<script>` の append によりブラウザが自動評価
   - `:full` ターゲットでは mruby parser が直接 evaluate
6. その後通常の `collect_definitions` + `expand_uses` + `Lilac.start` 本体

#### `:compiled` への適用

`:compiled` では `<script type="text/ruby">` をランタイムで評価できない
ため、bundle には `<template>` だけを含め、`.mrb` は別ファイルとして
配信する:

```
dist/
├── index.html               ← <link rel="lilac-bundle"> + bootstrap script
├── lilac.bundle.html        ← <template> だけ (no <script>)
├── app.<hash>.mrb           ← 全 .lil の script を統合した .mrb (既存)
└── vendor/lilac-compiled/
```
...
<script type="module" data-lilac-bootstrap>
  ...
  await boot({ bytecode: "./app.<hash>.mrb" });
</script>
```

ランタイム動作は `:full` とほぼ同じ。`<script type="text/ruby">` を append
する代わりに、boot が指定された `.mrb` を `loadBytecode` で読む。

#### ノービルド時の使い方

ノービルドでも同じ `<link rel="lilac-bundle">` 仕組みが使える。
`lilac.bundle.html` を手書きするだけ:
...
```html
<!-- components.html (手書き、複数 .lil の連結 = .lil の集合体) -->
<template>
...
`components.html` は **`.lil` の集合体**として書ける (`.lil` の形式が
そのまま HTML なので、ブラウザでも直接読める)。ノービルドユーザーが
「コンポーネント定義群を別ファイルに切り出す」ニーズに応えられる。

### 利点

- **HTML がクリーン** — ページ HTML はマークアップに専念、コンポーネント
  定義は別ファイル
- **HTTP/1 / HTTP/2 両対応** — bundle は 1 ファイルなのでリクエスト数が
  最小。HTTP/2 でも問題なく、HTTP/1 でも遅延が小さい
- **ビルドの責務が単純化** — `.lil` を連結するだけ。HTML 加工処理を大幅に
  削減 (~80 行削減見込み)
- **dev 体験が良い** — bundle.html 1 ファイル再生成 + リロード、watch /
  live reload が簡潔
- **キャッシュ単位がアプリ単位** — `.lil` を変更すると bundle.html が
  invalidate されるが、ページ HTML は影響なし
- **ノービルド世界観と互換** — `.lil` の集合体としての bundle.html を
  手書き可能、ADR-0029 の runtime canonical 原則を強化
- **共通 delivery 仕様** — `:full` / `:compiled` で同じ `<link rel="lilac-bundle">`
  を使う対称的な設計
...
HTTP/2 で並列ダウンロードできるが、bundle が大きいと wait time が増える。
緩和策: `<link rel="preload" as="fetch">` の併用、small bundle の維持。

**(2) キャッシュ粒度はアプリ全体**

1 つの `.lil` を変更すると bundle.html 全体が invalidate される。
component 単位でキャッシュしたい場合は、後述の代替案 (manifest + N 個) を
検討する。

**(3) `<script>` 自動評価のための DOM 操作**

`DOMParser` でパースした HTML 内の `<script>` は自動評価されないため、
runtime が新規 `<script>` 要素を作って append する必要がある。これは標準
の DOM API でできるが、実装上の注意点。

**(4) ノービルドで bundle.html を手書きする際の規約明文化**

`.lil` の連結 = bundle.html という形式なので、書き手は順番や重複に注意
する必要がある。ADR-0029 の衝突エラー (重複定義) は引き続き runtime で
catch される。

### 現状の workaround

不要 — 現状の inject 方式は引き続き動作する。本提案は新規 delivery オプション
の追加であり、既存方式を即時廃止する必要はない。
...
```json
{ "components": ["/components/counter.lil.html", "/components/todo-list.lil.html"] }
```

- 利点: コンポーネント単位のキャッシュ、HTTP/2 並列ダウンロード
- 欠点: HTTP/1 でリクエスト数が増える、ビルドの責務が増える (N 個出力 +
  manifest)、ノービルドユーザーが「全コンポーネントの URL リストを管理」
  する必要

**棄却理由**: 中規模アプリには過剰、HTTP/1 互換性も悪化。将来「lazy load」
ニーズが顕在化したら新規 ADR として検討する。

**案 C: HTML 内 inject 維持 + bundle オプション追加**

3 モード (inline / inject / bundle) を併存させる。

**棄却理由ではないが保留**: 段階的に移行するなら現実的だが、最初から
3 モード提供するのは複雑。当面は `:bundle` 一本に絞り、必要なら後で
オプション化する。
...
- 既存の `build_injection` から `default_templates` + `named_templates` +
  `<script>` bundle 処理を切り出して `lilac.bundle.html` に出力する
- ページ HTML には `<link rel="lilac-bundle" href="/lilac.bundle.html">` を
  注入する (head が無い HTML には head タグを追加)
- `:compiled` ターゲットでは `<script>` を bundle に含めず、現状通り `.mrb`
  にして別配信
- 推定: ~70 行削減 + ~30 行追加

**ランタイム (mruby-lilac)**:

- `Lilac.start` の最初に `<link rel="lilac-bundle">` を検出 + fetch
- `DOMParser` でパースして `<template>` を document.body に append
- `<script type="text/ruby">` を抽出 + 新規 `<script>` 要素として append
- fetch 失敗時のエラーハンドリング (`console.error` で報告)
- 推定: ~80 行追加

**設定 (Lilac.config)**:

- `c.delivery = :bundle` / `:inline` (current) で切り替え可能にする
- デフォルトは段階的移行のため当面 `:inline` (互換性維持)、安定したら
  `:bundle` をデフォルトに昇格
- 推定: ~20 行

**互換性 (破壊的変更ではない)**:

- 既存 `:inline` モードは引き続き動作
- `:bundle` を opt-in する形で導入
- 移行ガイド: scaffold の `lilac.config.rb` を `:bundle` 推奨に更新

**未解決の細部**:

- bundle.html 内の `<script>` 評価順序 (mruby は順次評価で OK?)
- bundle.html 内に複数の `<link rel="lilac-bundle">` を許可するか (= bundle
  の bundle、ネスト可)
- bundle.html を fetch 失敗時のフォールバック (`<noscript>` 案内?)
- `.lil` の packages 依存をどう bundle に含めるか (現状 package は別途
  `.mrb` + manifest で配信されているので、bundle とは独立)

### 関連する確定判断

- [ADR-0001](./adr/0001-runtime-canonical.md) — runtime canonical 原則。
  本提案は **canonical 原則を更に強化する** 方向 (delivery 形態を runtime
  が制御、build は単純な集約だけ担当)
- [ADR-0028](./adr/0028-drop-npm-distribution-github-pages-cdn.md) — GitHub
  Pages CDN 配信。bundle.html も同じ delivery 仕組みに乗せられる
- [ADR-0029](./adr/0029-data-component-data-use-split.md) — `data-component=` /
  `data-use=` の役割分離。本提案は ADR-0029 の delivery 戦略を完成させる
  位置付け

### ステータス

未判断 (proposal 段階)。実装の規模感は中規模 (CLI ~30 行追加 + ~70 行削減、
runtime ~80 行追加)。当面は既存の `:inline` モードを残しつつ `:bundle` を
opt-in で提供し、安定したらデフォルトを切り替える段階的移行案を推奨する。

「ビルドの役割を最小化」の中長期方針において、ADR-0029 の延長として
最も筋良い次の一歩。実装着手前に確認すべき判断軸:

- `:bundle` をデフォルトにするタイミング (当面 opt-in / 即デフォルト)
- 3 モード (inline / bundle / manifest) を将来許容するか、`:bundle` 一本に
  絞るか
- bundle.html の MIME type / 拡張子の規約 (`.html` で十分か、独自拡張子か)
