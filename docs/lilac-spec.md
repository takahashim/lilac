...
- `key:`: 必須。**`String`** (Hash subscript ショートカット) または **`Proc`**
  ...
  - **`Symbol` 渡しはエラー** (Lilac items は String キー規約のため、`key: :id` は `ArgumentError` で String 形を案内する)
...
| `Lilac::Template` | **template モード** — `template(name)` の戻り値、または `Lilac::Template.new(node)` で wrap した DOM 要素。同じ key で同じ underlying ノードが返ったら DOM 操作なし。違えば `replaceChild` |

mode は同じ key について最初の render で固定。途中で string ↔ template を切り替えると毎回 `replaceChild` になる。

**それ以外の戻り値は raise** :
- 生の `JS::Object` 要素を返した場合 → `Lilac::Error: bind_list block returned a raw JS::Object. Wrap it via Lilac::Template.new(node), or use the template(name) helper.`
- Hash / nil / Integer 等 → `Lilac::Error: bind_list block must return Lilac::Template, HTML::Safe, or String; got <ClassName>`
...
実例: `examples/lilac-receipt.html` (明細計算機)。
...
- 戻り値は `Lilac::Template`。bind_list の戻り値として直接渡せる
