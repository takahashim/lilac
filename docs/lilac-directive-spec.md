...
| `it.title` | dot 構文は廃止(bare ident で書く) |
| `title.upcase` | method chain |
| `save!` | bang は全 directive で禁止 |
| `user.name` | bare ident からの dot 禁止(field access は 1 段だけ) |
| `save?` (data-on-X) | event handler に predicate suffix 不可 |
| `title.length > 5` | 比較演算子 |
...
| `data-text="@s"` | ivar / bare_ident | `bind refs.gN, text: @s` |
| `data-unsafe-html="@s"` | ivar / bare_ident | `bind refs.gN, html: @s` |
| `data-bind="@s"` | ivar / bare_ident(両者とも writable Signal を指す必要あり) | `bind_input refs.gN, @s` ([§6.2](#62-data-bind--two-way-input-binding)) |
| `data-show="@s"` | ivar / bare_ident | lil-hidden を falsy 時に付与 |
| `data-hide="@s"` | ivar / bare_ident | lil-hidden を truthy 時に付与 |
| `data-on-X="m"` | method_ident | `refs.gN.on(:X) { \|ev\| m(...) }` |
| `data-attr-X="@s"` | ivar / bare_ident | `bind refs.gN, attr: { "X" => @s }` |
| `data-arg-X="@id"` | ivar (同要素に `data-component` 必須、**DOM 経由**) | `t.data(:X, @id.value)` (子は `root.data(:X)` で読む) |
| `data-prop-X="..."` | literal / ivar(同要素に `data-component` 必須、子に `prop :X` 宣言が必要。bare_ident は **literal として扱う** — iteration item field を渡したい場合は auto-fill 経由 [props-spec](./lilac-props-spec.md) 参照) | child の `@X` Signal を auto-init。子は `@X` / `props.X` / `instance.X` の 3 経路で値を読める。詳細 [`lilac-props-spec.md`](./lilac-props-spec.md) |
| `data-css-X="@s"` | ivar / bare_ident | `element.style.setProperty("--X", @s.to_s)` (CSS custom property 設定、`--` は自動 prepend) |
| `data-each="@c"` | ivar / bare_ident | `bind_list refs.gN, @c do \|it, t\| ... end` |
| `data-key="id"` | key_field (同要素に `data-each` 必須) | `key: ->(it) { it.id }` |
| `data-class="{a: @s}"` | hash_literal (value 側は ivar / bare_ident) | `bind refs.gN, class: { "a" => @s }` |
...
文法上は `ivar` / `bare_ident` を受けるが、**設計意図は id / status / type tag /
mode flag 等の short string を渡すこと**に限定する。

| 用途 | 適否 |
|---|---|
| iteration item の id (`data-arg-id="id"`) | ⭕ canonical |
| status / type tag (`data-arg-status="status"`) | ⭕ |
