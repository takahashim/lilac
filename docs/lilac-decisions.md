...
## 24. Plug-in 配布形態: `lilac-compiled` core + 個別 npm plug-in パッケージ
...
| Plug-in 配布形態: `lilac-compiled` core + 個別 npm plug-in(`mrb_load_irep` / `vm.loadBytecode` で後乗せ、`lilac plugin-build` subcommand で `.mrb` 生成、`mrbc-host.wasm` 再利用)— variant 爆発回避と modular 配布の両立 | (本 doc §24 が SSOT。実装は `cli/lib/lilac/cli/plugin_build.rb` + `npm/lilac-compiled/index.js` `boot({plugins})` hook + `npm/lilac-plugin-{extras,router,async}/` + `docs/lilac-plugin-spec.md`) | 完了 (2026-05-23) | §24 |
