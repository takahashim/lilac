...
| Plug-in 配布を rubygems に pivot — npm は `@takahashim/lilac-full` のみ (CDN お試し用途)、`lilac-compiled` も含めて plug-in 全部 rubygems、Bundler auto-discovery で zero-config。Lilac の Ruby 中心 self-identity と整合、mruby version lock 問題が原理的に消える | (本 doc §25 が SSOT。実装は `runtime/mruby-lilac-*/lilac-plugin-*.gemspec` + `cli/lib/lilac/cli/plugin_discovery.rb` + Builder 統合 + `--target full` 明示 boot 化 + npm/lilac-{compiled,plugin-*} 削除) | 着手 (2026-05-23) | §25 |
