...
個別の主要設計判断は **[`lilac-decisions.md`](./lilac-decisions.md)** を参照(粒度・更新頻度が異なるため別 doc に分離)。
...
- HTML 要素の生成は HTML で書く(Ruby builder DSL は提供しない、[lilac-decisions.md §5](./lilac-decisions.md))
...
主要設計判断の詳細(判断 / 背景 / rationale / トレードオフ)と年表は[`docs/lilac-decisions.md`](./lilac-decisions.md)を参照。
