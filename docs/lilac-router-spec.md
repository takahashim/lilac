...
複数ページ SPA 構成のための **`mruby-lilac-router`** gem の API 仕様。実装は `mrbgem/mruby-lilac-router/mrblib/lilac_router.rb`。`mruby-lilac` core に依存するが、core からは依存しない (opt-in)。
...
`window.location` (path / query / hash) を **signal として扱う** ことで、URL 状態を Lilac の reactivity の一部に組み込む。具体的には:
...
- **既存の `Lilac::Template` 機構を流用した lazy mount** を採用 (active route の `<template>` を outlet に clone、route 切替時に差し替え)
...
## Lilac Router を使わない選択 (MPA)

各 route が別 HTML ファイル (例: `/home.html`, `/users.html`) で、ページ遷移は通常の `<a href>` によるフルリロードで良い場合は **`mruby-lilac-router` を読み込まないのが正解**。各 HTML で `Lilac.start` だけ呼んで、その page 専用の component を mount すれば足りる。
...
のいずれかに該当する時だけ。MPA + Lilac 単体使用は、`mruby-lilac` core (Router gem 不要) で完結する。両者の住み分けは Router gem を読み込むかどうかで決まる。

---

## Router Context

すべての URL state / route 表 / outlet 設定 / listener は **`Lilac::Router::Context`** クラスのインスタンスが保持する。アプリ全体で 1 つの **default context** が `@default_context` として遅延生成され、これがアプリの Router 本体になる。

### アクセス方法

| 場所 | アクセス方法 |
|---|---|
| Boot コード (script トップレベル等、`draw`/`start` のみ) | `Lilac::Router.draw` / `Lilac::Router.start` の short-cut で default_context に forward |
| Boot コードでそれ以外を呼ぶ場合 | `Lilac::Router.default_context` を経由 (テストや低レベル操作) |
| Component 内 (`setup` etc.) | `Lilac::Router::WidgetMixin` が `router` instance method を提供。`lookup(:router)` 経由で取得するため、親 component が `expose :router, sub_ctx` で sub-context を注入できる |
| 追加 Context (sub-router など) | `Lilac::Router.new_context` で別 instance を生成 |

```ruby
# Boot — lifecycle short-cut
Lilac::Router.draw outlet: "[data-router-outlet]" do
  page :home, "/"
end
Lilac::Router.start

# Component — context は WidgetMixin の router 経由
class HomePage < Lilac::Component
  def setup
...
### `Lilac::Router` module-level API のスコープ

Module-level に提供される shortcut は **default context 限定**、かつ **boot 時に 1 回呼ぶ lifecycle 系のみ**:

| Module-level API | 役割 |
|---|---|
| `Lilac::Router.default_context` | 既定 Context を返す (lazy initialize) |
| `Lilac::Router.new_context` | 追加 Context を新規生成 |
| `Lilac::Router.draw(outlet:, &block)` | `default_context.draw(...)` への shortcut |
| `Lilac::Router.start(mode:, base:)` | `default_context.start(...)` への shortcut |
...
- Component 外: `Lilac::Router.default_context.foo` (テスト等)
...
`Lilac::Router` Module は **factory + namespace + 限定的 lifecycle shortcut** のみを提供し、状態は持たない。実際の状態は `Context` instance に閉じ込められている。これにより:
...
- **Sub-router**: 子 component tree に独自 Context を注入 (`expose :router, Lilac::Router.new_context`) すると、その subtree の `router.path` 等は別 Context を見る。outlet を sub-context に紐付ければネスト route が実現可能 (v2 候補)
- **複数の独立 Router**: 1 HTML 内に独立した複数 SPA がある稀なケースで `Lilac::Router.new_context` を使う
...
低レベル API は単独で完結しているので、「URL から signal を読みたいだけ」「常時マウント + `bind hidden:` でページ切替したい」用途では DSL 不要。`mrbgem/mruby-lilac-router` は両方を提供する。
...
- outlet 内の `data-component="..."` は既存 Lilac の MutationObserver により自動で mount / unmount
...
Lilac::Router.start  # mode: :hash がデフォルト
```
...
class UserDetail < Lilac::Component
  def setup
...
`Lilac.start` 後 (もしくは前) に以下を呼ぶ:

```ruby
# 低レベルのみ
Lilac::Router.start(mode: :hash, base: "/")
...
`Lilac::Router.draw` / `start` は default context への lifecycle shortcut。Component 内では `Lilac::Router::WidgetMixin` が同じ Context を `lookup(:router)` 経由で `router` として公開する (= 後述の Component 内 API と一貫)。
...
```ruby
Lilac::Router.start(mode: :history, base: "/myapp/")
```
...
5. Component 経由 (`component.bind_link(...)` = `Lilac::Router::WidgetMixin`) で呼ばれた場合、effect は component 寿命に紐付き unmount で auto-dispose
...
`mruby-lilac-form` の `form` ヘルパと同じパターンで、`Lilac::Router::WidgetMixin` が `Lilac::Component` に include される。これにより:
...
```ruby
class SearchPage < Lilac::Component
  def setup
...
```ruby
Lilac::Router.default_context.start
...
class Nav < Lilac::Component   # 常時マウント
  def setup
    # 常駐、各 link の active 強調を router.current で
  end
end

class Footer < Lilac::Component  # 常時マウント
  def setup
    # ...
  end
end
...
### `Lilac::Template` 機構の流用

DSL の lazy mount は内部で `Lilac::Template` の HTML `<template>` clone 機構を流用する。Router 側に独自実装は持たず、`Lilac::Template` の API を call するだけ。

### MutationObserver による component auto-mount

clone した template を outlet に append すると、内部の `data-component="..."` は既存 Lilac の MutationObserver により自動で component mount される。前 route の DOM を取り除けば component は auto-cleanup する。Router 自身は component lifecycle を管理しない。
...
```ruby
class TodosPage < Lilac::Component
  def setup
...
### mruby-wasm-js / mruby-lilac core への要求

- mruby-wasm-js: 追加 import なし (`JS.global[:window]` / `:history` / `:location` の既存ブリッジで完結)
- mruby-lilac (core): 変更なし。`Lilac::Router` は core の `signal` / `computed` / `effect` / `Lilac::Template` を消費するだけ
...
- **既存 `Lilac::Template` インフラを流用**: 自前 DOM 操作なしで実装できる
- **state 保持が必要なら `persistent_signal`**: 既存 Lilac primitive で対応可
...
- Lilac で SPA navigate しか提供しない以上、HTTP verb を借りる意味がない
...
明示呼び出しなら `root.on(:click) { |e| router.intercept_link(e) }` 1 行で副作用範囲を nav 内に限定できる。Lilac 全体の "magic を避ける" 哲学に整合。
...
# Component 内 (Lilac::Router::WidgetMixin が同じ Context を `router` として expose)
# 自動生成 API (DSL 経由)
router.home_path                # => "/"
router.user_path(id: 42)        # => "/users/42"
...
`{ path:, query:, hash: }` を毎回新しい Hash で生成すると、ハッシュ全体が変わり path のみ依存している computed も全部再計算される。signal は前回値と `==` 比較で短絡する設計を確認 (mruby-lilac core の Signal 実装を要 audit)。必要なら `path` / `query` / `hash` 個別 signal を内部で持って `location` は computed として合成する。
