...
| doc | 内容 | 読むべきとき | 更新頻度 |
|---|---|---|---|
| [`lilac-decisions.md`](./lilac-decisions.md) | 確定した個別判断の log。§1〜§N の通し番号、覆された判断も `(superseded by §N)` で残す | spec を読んで「なぜここはこうなっているか」を引きたいとき | 決定単位で追記、削除はほぼなし |
| [`lilac-proposals.md`](./lilac-proposals.md) | **未確定** の設計提案。確定したら decisions.md に昇格して削除 | 議論中の変更を把握 / 新提案を起こす前の重複チェック | 任意のタイミングで編集可 |
...
| [`lilac-plugin-spec.md`](./lilac-plugin-spec.md) | Plug-in 機構: 公式 / 第三者 plug-in の書き方・ビルド・配布・load | `mruby-lilac-directives`(register surface)|
...
[proposals.md](./lilac-proposals.md) で議論 → 確定 → [decisions.md](./lilac-decisions.md) に新 §N + 影響 spec 更新 → 原則改訂を伴うなら [design.md](./lilac-design.md) も同期
...
- decisions は積み上げ
  - 削除せず、覆すときは `(superseded by §N)` でマーク。歴史が doc 内で完結することが価値
- proposals は流動的
  - 議論で内容が変わってよい。却下なら削除、確定したらdecisions.md に移植。昇格 operation は proposals.md 冒頭参照
- design.md は最少改訂
  - 原則そのものを書き換えるレベルだけ。個別判断はdecisions.md 側で記録する
