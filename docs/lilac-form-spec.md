...
- **input binding 自体は form 非依存** ([decisions §21](./lilac-decisions.md))。
  ...
  「その上に validation 等を載せたい」場合に使う optional 集約 layer
...
- `lilac-compiled` でも validation / submit / reset / base_error の
  declarative API が動かないと実用 form を組めない
- 「form は便利機能の1つ」ではなく「Lilac の **validation / submit 集約
  layer**」(decisions §21)。input binding 自体は `data-bind` が
  canonical なので form gem 無しでも動くが、validation などを欲しい瞬間に
  必須となる中核 component
...
iteration item の Signal field を bind する形で書く。flat な Symbol → Field
map は維持しつつ、コレクション内 input の declarative binding は data-bind
が担う構成。

上記の真の意味でのネスト構造 / fields_for 風 sub-form は依然
スコープ外で、必要なユースケース(複数の同型 sub-form を増減 etc.)は
...
### 11.8 廃止された旧 directive と `data-bind` への統合

- `data-value="@signal"` — **廃止**。後継として `data-bind="@signal"`
  (directive-spec §6.2、decisions §21)が input type に依らず統一形で
  提供される
- `data-checked="@signal"` — **廃止**。同じく `data-bind="@signal"` が
  input type=checkbox を自動 detect して `:checked` プロパティを sync

両者は §2 (現 decisions §2) で「form を中心に据える」方針のもと一度
廃止されたが、運用上「コレクション内 input / 単発 toggle / 検索ボックス
等で form 経由は unergonomic」という摩擦が顕在化し、decisions §21 で
**form 非依存の two-way binding directive として `data-bind` に統合再生**
された。詳細経緯は decisions §21 を参照。

migration:

- 旧 `<input data-value="@query">` → 新 `<input data-bind="@query">`
  (validation 不要な単発 input)
- 旧 `<input data-value="@query">` で validation も欲しい場合 →
  `f.field :query, initial: ""` を Ruby に追加 +
  `<input data-field="query">` に変更(form 経由)
- 旧 `<input data-checked="@flag">` → 新 `<input type="checkbox" data-bind="@flag">`

## 12. parity(declarative vs imperative)

利用者が場面に応じて選べる 3 経路。すべて最終的には同じ
`Effect` / `bind_input` / `bind` 呼び出しに帰着する(decisions §21 の
3 層モデル: signal / binding / form)。

### binding 単独 (form features 不要 / `data-bind`)

「DOM 結線だけ欲しい、validation も submit も要らない」最小ケース。
form は立てない、単発 toggle や検索ボックスやコレクション内 input 向け。
...
`data-bind` は input type を自動 detect し、value / checked / files の
どれを sync するか決める(directive-spec §6.2)。

### imperative (form + Ruby DSL、`data-field` なし)

form は使うが、wiring は Ruby 側で `f.field :name, ref:` + 手書きの
`bind refs.X, ...` を使う形。declarative directive を使わない場合の経路。
...
3 経路で観察される DOM 変化は同一(parity test で担保)。
