...
| ~~`data-prop-x="it.field"`~~ | iteration item の `field` → `.to_s`(**DEPRECATED** — §7.6 auto-fill に移行) |
| ~~`data-prop-x="it"`~~ | iteration item 全体 → `.to_s`(**DEPRECATED**) |
| `data-prop-x="bare_ident"` | **literal として扱う**(bare ident は他 directive では iteration item field を意味するが、`data-prop-*` だけは literal fallback を優先) |
...
主な用途は `<ul data-each="@items"> <li data-component="Row" data-prop-id="it.id"
data-prop-title="it.title">` のように iteration row を component に切り出す
パターンだった。これは現在 §7.6 の **prop auto-fill** で書く方が簡潔
(`data-prop-id="it.id"` 等の forest を撤去できる)。`it.path` 形は
dev_mode で deprecation warn が出る。

**制約**:
- `it.X` を解決するには **parent が data-each 内**である必要 (= parent の
  per-row scanner が `item` を持っている文脈)
