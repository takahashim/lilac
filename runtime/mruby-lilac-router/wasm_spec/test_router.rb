...
def router
  Lilac::Router.default_context
end

# ---------- Path / match (low-level) ----------

Spec.describe "Lilac::Router low-level: location/path/match" do
...
Spec.describe "router.draw + page DSL" do
  ...
  Spec.assert "params returns active route params (auto-tracks location)" do
    ...
    seen = []
    m = Lilac::Computed.new { router.params[:id] }
    seen << m.value
...
Spec.describe "Lilac::Router link helpers" do
  Spec.assert "default_context is stable and delegates generated path helpers" do
    ...
    c1 = Lilac::Router.default_context
    c2 = Lilac::Router.default_context
    Spec.assert_true c1.equal?(c2)
    Spec.assert_true c1.respond_to?(:user_path)
    Spec.assert_false Lilac::Router.respond_to?(:user_path)
  ...
  Spec.assert "new_context keeps generated helpers separate from default_context" do
    router.__reset_for_tests__
    context = Lilac::Router.new_context
    ...
    Spec.assert_true context.is_a?(Lilac::Router::Context)
  ...
  Spec.assert "bind_link writes href and toggles active / inactive classes" do
    ...
    # the component mixin, or the Lilac::Effect lives until VM teardown
  ...
  Spec.assert "ComponentMixin#router returns default_context without provider" do
    doc = JS.global[:document]
    component = Class.new(Lilac::Component).new(doc.call(:createElement, "div"))
    Spec.assert_true component.router.equal?(Lilac::Router.default_context)
  end

  Spec.assert "ComponentMixin#router uses exposed router context" do
    doc = JS.global[:document]
    custom_router = Object.new
    parent = Class.new(Lilac::Component) do
      define_method(:prepare_setup) { expose :router, custom_router }
    end.new(doc.call(:createElement, "div"))
    child = Class.new(Lilac::Component).new(doc.call(:createElement, "div"))
...
Spec.describe "Lilac::Router lazy mount" do
...
Spec.describe "Lilac::Router bootstrap" do
