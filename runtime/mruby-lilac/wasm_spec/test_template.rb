Spec.describe "Lilac.template" do
  Spec.assert "returns a Lilac::Template wrapping the first element child" do
    body = JS.global[:document][:body]
    body[:innerHTML] = '<template data-template="t1"><div class="ok"></div></template>'
    t = Lilac.template("t1")
    Spec.assert_true t.is_a?(Lilac::Template)
  ...
  Spec.assert "valid name shapes (letter / digit / underscore / hyphen) are accepted" do
    ...
    HTML
    t = Lilac.template("row_with-name7")
    t.refs.x.text = "a"
...
Spec.describe "bind_list with Template" do
  ...
  Spec.assert "raw JS::Object return raises clear error" do
    ...
    captured = []
    Lilac.logger = ->(_severity, label, err) { captured << [label, err] }
    ...
    Lilac.register "bl-tmpl-raw", klass
    Lilac.start
  ...
  Spec.assert "non-Template / non-String return raises clear error" do
    ...
    Lilac.register "bl-tmpl-bogus", klass
    Lilac.start
