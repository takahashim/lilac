# lilac_component.rb — Lilac::Component base class.
#
# Users write `class Counter < Lilac::Component`. Owns lifecycle
# (prepare_setup / setup / cleanup), reactive helpers (signal / computed /
# effect / persistent_signal), error boundary, and expose / lookup.
#
# Bindable mixin (in lilac_bindable.rb) supplies bind / bind_input /
# bind_list. Registry (in lilac_registry.rb) drives mount / unmount.
...
module Lilac
  ...
  # `Lilac.logger.error` with `source:` set to the owning Component, so
  ...
  class DisposableSet
    ...
    def safe_release(label)
      ...
      Lilac.logger.error("#{@source.class.name} #{label}", e, source: @source)
    end
  end
  ...
  # The user-inheritable base. Users write `class Counter < Lilac::Component`.
  class Component
    ...
    # component subtree at mount time and wires the `bind` / `bind_input`
    # / `bind_list` calls for each `data-*` directive. Only active in
    # builds that include the directives gem (every shipped target does).
    #
    # A component may override `bind_template_hook` to wire bindings
    # imperatively instead; the override replaces this default entirely.
    #
    ...
    def lookup(key, default = NOT_FOUND, &block)
      ...
      raise Lilac::Error, "lookup: no exposed value for #{key.inspect} in #{self.class.name}"
    end
    ...
    def each_frame(&block)
      ...
      cb = JS.callback do |ts|
        ...
        begin
          ...
          Lilac.logger.error("each_frame", e, source: self)
        end
    ...
    # milliseconds. Returns a `Lilac::Timer` — call `.stop` on it to
    ...
    def timeout(ms, &block)
      ...
      cb = JS.callback do
        begin
          ...
          Lilac.logger.error("timeout", e, source: self)
        end
      end
    ...
    # Returns a `Lilac::Timer`; call `.stop` to stop early.
    def every(ms, &block)
      raise ArgumentError, "block required" unless block
      cb = JS.callback do
        begin
          ...
          Lilac.logger.error("every", e, source: self)
        end
      end
    ...
    # when not using `Lilac::Aborted` flow.
    def alive?
      !@unmounted
    end
    ...
    # the yield. Raises `Lilac::Aborted` (silenced by framework) if
    ...
    # See docs/lilac-spec.md "Error Boundary".
    def on_error(&block)
      raise ArgumentError, "block required" unless block
      @error_handler = block
    end
    ...
    def handle_error(label, error)
      ...
      begin
        ...
        Lilac.logger.error("on_error handler in #{self.class.name}", e)
        false
      end
    end
    ...
    def prepare_setup_phase
      ...
      begin
        ...
        Lilac.logger.error("#{self.class.name}#prepare_setup", e, source: self)
      end
    end
