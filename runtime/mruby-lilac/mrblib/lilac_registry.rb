...
module Lilac
  ...
  class Registry
    ...
    def start(root_js = nil)
      ...
      # `<link rel="lilac-bundle">` resources are fetched and merged by
      # the boot helper (e.g. boot.js / pages/lilac-full/index.js) BEFORE
      # vm.eval("Lilac.start"). The boot helper handles fetch + DOMParser
      # + `<script type=text/ruby>` evaluation through `vm.eval` because
      # mruby-eval (`Kernel#eval`) isn't included in lilac-full. By the
      # time this method runs, every bundle's templates and scripts are
      # already in the live document.
