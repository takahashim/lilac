# Tests for `Lilac::Resource.current_run` + Fetchy auto-injection of
...
Spec.describe "Lilac::Resource.current_run + Fetchy signal injection" do
  ...
  Spec.assert "Fetchy inside resource block auto-uses run.abort_signal" do
    ...
    Lilac.register "inject-basic", klass
    Lilac.start
  ...
  Spec.assert "explicit signal: overrides the auto-injected one" do
    ...
    Lilac.register "inject-override", klass
    Lilac.start

    inst = Lilac.find_for_element(doc.call(:querySelector, "[data-component='inject-override']"))
    ...
    klass = Class.new(Lilac::Component) do
      define_method(:setup) do
        @ok = resource(initial: nil) do
          seen_inside << Lilac::Resource.current_run
          "done"
        end
      end
    end
