...
#     bridge protocol used by mruby-wasm-js (Lilac) — dispatches into
#     the same underlying Ruby methods.
module Dommy
  ...
  class Window
    ...
    def __js_get__(key)
      ...
      when "customElements" then @custom_elements
      when "navigator"      then @navigator
      else @globals[key]
      end
    end
